...
function firstLine()
{
LCD_print(1, "BeagleBone Load", doUpdate);
}
function doUpdate()
{
fs.readFile("/proc/loadavg", writeUpdate);
}
function writeUpdate(err, data)
{
LCD_print(2, data.toString().substring(0, 14), onUpdate);
}
function onUpdate()
{
...
//to LCD
if((chr & 0x0080)== 0x0080) b.digitalWrite(LCD_DB7, b.HIGH, next);
  else b.digitalWrite(LCD_DB7, b.LOW, next);
},function() {
if((chr & 0x0040)== 0x0040) b.digitalWrite(LCD_DB6, b.HIGH, next);
  else b.digitalWrite(LCD_DB6, b.LOW, next);
},function() {
if((chr & 0x0020)== 0x0020) b.digitalWrite(LCD_DB5, b.HIGH, next);
  else b.digitalWrite(LCD_DB5, b.LOW, next);
},function() {
if((chr & 0x0010)== 0x0010) b.digitalWrite(LCD_DB4, b.HIGH, next);
  else b.digitalWrite(LCD_DB4, b.LOW, next);
},function() {
if((chr & 0x0008)== 0x0008) b.digitalWrite(LCD_DB3, b.HIGH, next);
  else b.digitalWrite(LCD_DB3, b.LOW, next);
},function() {
if((chr & 0x0004)== 0x0004) b.digitalWrite(LCD_DB2, b.HIGH, next);
  else b.digitalWrite(LCD_DB2, b.LOW, next);
},function() {
if((chr & 0x0002)== 0x0002) b.digitalWrite(LCD_DB1, b.HIGH, next);
  else b.digitalWrite(LCD_DB1, b.LOW, next);
},function() {
if((chr & 0x0001)== 0x0001) b.digitalWrite(LCD_DB0, b.HIGH, next);
  else b.digitalWrite(LCD_DB0, b.LOW, next);
},function() {
//LCD Register Set (RS) to logic zero for command input
b.digitalWrite(LCD_RS, b.LOW, next);
},function() {
//LCD Enable (E) pin high
b.digitalWrite(LCD_E, b.HIGH, next);
},function() {
//End the write after 1ms
setTimeout(next, 1);
},function() {
  //LCD Enable (E) pin low and call scheduleCallback when done
  b.digitalWrite(LCD_E, b.LOW, next);
},function() {
...
b.digitalWrite(LCD_RS, b.HIGH, next);
},function() {
//LCD Enable (E) pin high
b.digitalWrite(LCD_E, b.HIGH, next);
},function() {
